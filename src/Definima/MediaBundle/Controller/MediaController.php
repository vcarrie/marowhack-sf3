...
namespace Definima\MediaBundle\Controller;

use Definima\MediaBundle\Event\FileManagerEvents;
use Definima\MediaBundle\Helpers\File;
use Definima\MediaBundle\Helpers\FileManager;
use Definima\MediaBundle\Helpers\UploadHandler;
use Definima\MediaBundle\Twig\OrderExtension;
    ...
    public function uploadFileAction(Request $request, $idUser = 0)
    {
        ...
        foreach ($response['files'] as &$file) {
            if (isset($file->error)) {
                ...
                $retour['files'][] = array('conf' => $queryParameters['conf'], 'url' => $file->name, 'name' => $file->name, 'nom' => strtolower(preg_replace("/[A-Z]+/", "_$0", $queryParameters['nom'])), 'size' => $file->size, 'type' => $file->type);
            }
