...
//        }
        ...
        $link = 'Aucun fichier...';
        if($filePath) {
            $link = '<a href="'.$this->router->generate($route, array('path' => $conf.'__'.basename($filePath))).'__'.strtolower(preg_replace("/[A-Z]+/", "_$0", $nom)).'" >Voir le document</a>';
        }
        ...
        switch (true) {
            ...
            case preg_match('/(gif|png|jpe?g|svg)$/i', $extension):
              ...
              break;
                /*return [
                    'path' => $filePath,
                    'html' => "<img src=\"".$this->router->generate($route, array('path' => $conf.'__'.basename($filePath))).'__'.strtolower(preg_replace("/[A-Z]+/", "_$0", $nom))."\" height='{$size}'>".$link,
                ];*/
